...
def _partial_path(out_dir: Path, target: str) -> Path:
    return out_dir / "benchmarks_partial" / f"{target}.json"


def save_partial(rows: list[dict], out_dir: Path, target: str) -> None:
    """Per-target results are saved as soon as the target finishes, so a killed run loses at most one target."""
    p = _partial_path(out_dir, target)
    p.parent.mkdir(parents=True, exist_ok=True)
    p.write_text(json.dumps(rows, indent=2, default=float), encoding="utf-8")


def load_all_partials(out_dir: Path) -> list[dict]:
    rows = []
    for t in C.TARGETS:
        p = _partial_path(out_dir, t)
        if p.exists():
...
def main() -> int:
    ...
    ap.add_argument("--targets", nargs="+", default=list(C.TARGETS), choices=list(C.TARGETS),
                    help="targets to (re)run; the report merges saved results of all targets")
    ...
    for t in args.targets:
        ...
        save_partial(rows, args.out_dir, t)
    # the report always merges every target that has results on disk (this run or earlier ones)
    write_report(load_all_partials(args.out_dir), args.out_dir, args)
    return 0
